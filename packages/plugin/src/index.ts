...
export const executePluginCommand = async (
  ...
  if (!(await fileExists(pluginPath))) {
    throw new ZaoWuError({
      code: 'PLUGIN_NOT_INSTALLED',
      message: 'Plugin is not installed.',
      why: `ZaoWu could not find the installed plugin manifest at \`${pluginPath}\`.`,
      fix: 'Install the plugin first using `zw plugin install <id>`.',
    });
  }

  let pluginRecord: PluginManifest;
  try {
    pluginRecord = JSON.parse(await readFile(pluginPath, 'utf8')) as PluginManifest;
  } catch {
    throw new ZaoWuError({
      code: 'PLUGIN_NOT_INSTALLED',
      message: 'Installed plugin metadata is unreadable.',
      why: `The file at \`${pluginPath}\` is not valid JSON.`,
      fix: 'Reinstall the plugin.',
    });
  }

  const sourceValidation = await validatePluginSource(pluginRecord.source);
  if (sourceValidation.status === 'warning' || !sourceValidation.manifest) {
    throw new ZaoWuError({
      code: 'PLUGIN_SOURCE_INVALID',
      message: 'Plugin source is invalid or missing manifest.',
      why: sourceValidation.errors.join(' ') || 'Could not parse manifest at plugin source.',
      fix: 'Fix the plugin source or manifest.',
    });
  }

  const manifest = sourceValidation.manifest;
  const commandDef = manifest.commands?.find((c) => c.name === command);
  if (!commandDef) {
    throw new ZaoWuError({
      code: 'UNKNOWN_COMMAND',
      message: `Command \`${command}\` is not defined by plugin \`${id}\`.`,
      why: `The plugin manifest does not declare command \`${command}\`.`,
      fix: `Check the available commands using \`zw plugin list\` or update the manifest.`,
    });
  }

  const resolvedPath = sourceValidation.manifestPath
    ? path.join(path.dirname(sourceValidation.manifestPath), 'bin', command)
    : path.join(pluginDir, id, 'bin', command);
  ...
  return {
    ...
    args,
    resolvedPath,
    output: `Mock executed plugin \`${id}\` command \`${command}\` with args: ${args.join(', ')}`,
  };
};
